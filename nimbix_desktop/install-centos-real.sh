...
yum -y install xorg-x11-fonts-ISO8859-1-100dpi xorg-x11-fonts-ISO8859-1-75dpi compat-libstdc++-33 python-pip ImageMagick-devel xorg-x11-apps xterm
...
#rm -f /usr/bin/Xvnc   # done by VNC install
yum -y install /tmp/VNC-Server-*-Linux-x64.rpm || \
    yum -y update /tmp/VNC-Server-*-Linux-x64.rpm
rm -f /tmp/VNC-*.rpm
#ln -sf /usr/bin/Xvnc-realvnc /usr/bin/Xvnc  # unneeded?
